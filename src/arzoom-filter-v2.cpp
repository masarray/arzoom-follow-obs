...
/* Keep the persisted values compatible with v0.1.x. The labels now describe
 * the Phase 1 camera character: Cinematic / Balanced / Responsive. */
...
enum class FollowMode { Smart, Centered, Fixed };
enum class MovementStyle { Cinematic, Balanced, Responsive };
...
#else
std::vector<MonitorDescriptor> enumerate_monitors() { return {}; }
bool get_cursor_position(long &, long &) { return false; }
#endif
...
MovementStyle parse_movement(const char *value)
{
    ...
    if (value && std::strcmp(value, MOVEMENT_FAST) == 0)
        return MovementStyle::Responsive;
    return MovementStyle::Cinematic;
}

arzoom::CameraFollowPolicy camera_follow_policy(FollowMode mode)
{
    switch (mode) {
    case FollowMode::Centered:
        return arzoom::CameraFollowPolicy::Centered;
    case FollowMode::Fixed:
        return arzoom::CameraFollowPolicy::Fixed;
    case FollowMode::Smart:
    default:
        return arzoom::CameraFollowPolicy::Smart;
    }
}

arzoom::CameraMotionStyle camera_motion_style(MovementStyle style)
{
    switch (style) {
    case MovementStyle::Responsive:
        return arzoom::CameraMotionStyle::Responsive;
    case MovementStyle::Balanced:
        return arzoom::CameraMotionStyle::Balanced;
    case MovementStyle::Cinematic:
    default:
        return arzoom::CameraMotionStyle::Cinematic;
    }
}
...
struct ArZoomFilter {
    ...
    std::atomic<int> movement_style{static_cast<int>(MovementStyle::Cinematic)};
...
void global_hotkey_toggle(void *, obs_hotkey_id, obs_hotkey_t *, bool pressed)
{
    ...
    for (ArZoomFilter *filter : filter_registry) {
        ...
        if (has_showing_filter && !obs_source_showing(filter->context))
            continue;
        filter->requested_zoom.store(next_zoom, std::memory_order_release);
    }
}
...
int hotkeys_page_row(QListWidget *navigation)
{
    if (!navigation)
        return -1;
    for (int row = 0; row < navigation->count(); ++row) {
        ...
        if (item && item->text().contains(
                        QStringLiteral("hotkey"), Qt::CaseInsensitive)) {
            return row;
        }
    }
    return navigation->count() > 6 ? 6 : -1;
}
...
bool focus_existing_hotkeys_page()
{
    ...
    for (QWidget *widget : widgets) {
        ...
        const int row = hotkeys_page_row(navigation);
        if (row < 0)
            continue;
...
void retry_focus_hotkeys_page(QWidget *context, int attempts)
{
    if (focus_existing_hotkeys_page() || attempts <= 0 || !context)
        return;
    QTimer::singleShot(50, context, [context, attempts]() {
        retry_focus_hotkeys_page(context, attempts - 1);
    });
}
...
bool open_hotkeys_settings()
{
    ...
    if (QAction *settings_action = main_window->findChild<QAction *>(
            QStringLiteral("action_Settings"))) {
...
bool resolve_monitor(ArZoomFilter *filter)
{
    ...
    if (target) {
        ...
        if (settings) {
            if (obs_data_has_user_value(settings, "capture_cursor")) {
                ...
                if (!captures_cursor &&
                    ...
                    blog(LOG_WARNING,
                         ...
                         "Smart Camera still tracks it, but viewers may not "
                         "see the pointer.");
            ...
            if (obs_data_has_user_value(settings, "monitor")) {
                const int monitor_index = static_cast<int>(
                    obs_data_get_int(settings, "monitor"));
                if (monitor_index >= 0 &&
    ...
    const auto primary = std::find_if(
        monitors.begin(), monitors.end(),
        [](const MonitorDescriptor &candidate) { return candidate.primary; });
    filter->monitor = primary != monitors.end() ? *primary : monitors.front();
                               ...
                               bool zoom_active)
{
    ...
    if (zoom_active && filter->monitor_refresh_elapsed >= 5.0f)
        filter->monitor_dirty.store(true, std::memory_order_release);
    if (!filter->monitor_dirty.exchange(false, std::memory_order_acq_rel))
        return;
    ...
    if (!filter->monitor_valid && !filter->monitor_warning_logged) {
        blog(LOG_WARNING,
             ...
             "safe; mouse intent is held until mapping is ready.");
...
void update(void *data, obs_data_t *settings)
{
        ...
        obs_data_get_string(settings, SETTING_TARGET_MONITOR);
    {
        ...
        filter->configured_monitor =
            target_monitor && *target_monitor ? target_monitor : MONITOR_AUTO;
    }
    filter->monitor_dirty.store(true, std::memory_order_release);
...
void defaults(obs_data_t *settings)
{
    ...
    obs_data_set_default_string(settings, SETTING_FOLLOW_MODE, FOLLOW_SMART);
    obs_data_set_default_string(settings, SETTING_MOVEMENT, MOVEMENT_BALANCED);
    ...
    obs_data_set_default_string(settings, SETTING_TARGET_MONITOR, MONITOR_AUTO);
...
obs_properties_t *properties(void *data)
{
    ...
    obs_property_t *follow = obs_properties_add_list(
        props, SETTING_FOLLOW_MODE, obs_module_text("ArZoom.MouseFollow"),
    ...
    obs_property_t *movement = obs_properties_add_list(
        props, SETTING_MOVEMENT, obs_module_text("ArZoom.Movement"),
        OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
    obs_property_list_add_string(
        movement, obs_module_text("Movement.Smooth"), MOVEMENT_SMOOTH);
    obs_property_list_add_string(
        movement, obs_module_text("Movement.Balanced"), MOVEMENT_BALANCED);
    obs_property_list_add_string(
        movement, obs_module_text("Movement.Fast"), MOVEMENT_FAST);

    obs_property_t *safe_zone = obs_properties_add_int_slider(
        props, SETTING_SAFE_ZONE, obs_module_text("ArZoom.SafeZone"),
        10, 60, 1);
    ...
    obs_property_t *anchor_x = obs_properties_add_int_slider(
        advanced, SETTING_ANCHOR_X, obs_module_text("ArZoom.AnchorX"),
        20, 80, 1);
    obs_property_int_set_suffix(anchor_x, " %");
    obs_property_t *anchor_y = obs_properties_add_int_slider(
        advanced, SETTING_ANCHOR_Y, obs_module_text("ArZoom.AnchorY"),
        20, 80, 1);
...
void tick(void *data, float seconds)
{
    ...
    const bool wants_zoom = is_enabled &&
        filter->requested_zoom.load(std::memory_order_acquire);

    const bool zoom_active = wants_zoom || filter->current_zoom > 1.001f;
    ...
    input.safe_zone = filter->safe_zone.load(std::memory_order_acquire);
    ...
    if (input.follow_policy != arzoom::CameraFollowPolicy::Fixed &&
        filter->monitor_valid) {
        long cursor_x = 0;
        long cursor_y = 0;
        if (get_cursor_position(cursor_x, cursor_y) &&
            cursor_in_monitor(filter->monitor, cursor_x, cursor_y)) {
            input.cursor = cursor_normalized(
                filter->monitor, cursor_x, cursor_y);
            input.cursor_valid = true;
        }
    }

    const arzoom::CameraOutput output = filter->camera.step(input);
    filter->current_zoom = output.zoom;
    filter->current_center = output.center;
}
...
void render(void *data, gs_effect_t *)
{
    ...
    vec2 center;
    vec2_set(&center, filter->current_center.x, filter->current_center.y);
...
void *create(obs_data_t *settings, obs_source_t *context)
{
    ...
    if (!effect_path) {
        filter->runtime_error = obs_module_text("ArZoom.Status.EffectMissing");
        blog(LOG_ERROR,
        ...
        if (filter->effect) {
            filter->zoom_param = gs_effect_get_param_by_name(
                filter->effect, "zoom_amount");
            filter->center_param = gs_effect_get_param_by_name(
                filter->effect, "zoom_center");
        }
        obs_leave_graphics();
        ...
        if (!filter->effect_ready) {
            if (filter->runtime_error.empty())
                filter->runtime_error =
                    obs_module_text("ArZoom.Status.ShaderUnavailable");
            blog(LOG_ERROR,
                 "[ArZoom] Zoom shader unavailable; safe pass-through active.");
        } else {
            blog(LOG_INFO, "[ArZoom] Smart Camera shader ready");
        }
    }
...
bool arzoom_register_global_hotkey()
{
    ...
    global_toggle_hotkey = obs_hotkey_register_frontend(
        HOTKEY_CONFIG_NAME, obs_module_text("ArZoom.Hotkey.Toggle"),
        global_hotkey_toggle, nullptr);
    if (global_toggle_hotkey == OBS_INVALID_HOTKEY_ID) {
        blog(LOG_ERROR, "[ArZoom] Failed to register global hotkey");
        return false;
    }
    ...
    load_hotkey_from_profile();
    blog(LOG_INFO,
         "[ArZoom] Global hotkey registered with profile persistence");
    return true;
}
